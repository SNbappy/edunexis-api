...
public record SyncUserCommand(string Email, string FullName) : ICommand<ApiResponse<UserDto>>;
...
public sealed class SyncUserCommandHandler(IUnitOfWork uow) : ICommandHandler<SyncUserCommand, ApiResponse<UserDto>>
{
    public async ValueTask<ApiResponse<UserDto>> Handle(SyncUserCommand command, CancellationToken ct)
    {
        ...
        return ApiResponse<UserDto>.Ok(new UserDto(
            ...
            UpdateProfileCommandHandler.MapToDto(profile)), "User synced successfully.");
    }
}
